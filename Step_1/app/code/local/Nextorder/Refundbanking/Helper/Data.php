...
class Nextorder_Refundbanking_Helper_Data extends Mage_Core_Helper_Abstract{
		...
		public function setKontoInfos($iban, $bic){
			...
			$customer_data->setData('iban',$iban)->setData('bic',$bic)->save();
		}
		...
		public function getSepaXML($name_kunde, $kundennr,$iban_kunde, $bic_kunde, $name_shop, $iban_shop, $bic_shop, $orderNr,$creditmemoInkreID ,$refundSumme){
			...
			if(!is_dir($base_path."/media/Sepa_Gtuschrift")) {
				mkdir($base_path . "/media/Sepa_Gtuschrift", 0777);
			}

			$sepaXML = $this->ini_Sepa_XML($name_kunde, $kundennr,$iban_kunde, $bic_kunde, $name_shop ,$iban_shop, $bic_shop, $orderNr, $refundSumme);

		file_put_contents($base_path."/media/Sepa_Gtuschrift/Gutschrift_".$creditmemoInkreID.".xml", $sepaXML);
			//$xmlURL = Mage::getUrl('media') . "Sepa_Gtuschrift/Gutschrift_".$orderNr.".txt";
//			$rootLink = str_replace("/index.php","",Mage::getUrl('media'));
//			$xmlURL = $rootLink ."Sepa_Gtuschrift/Gutschrift_".$orderNr.".xml";
//			return $xmlURL;
		}

//		public function getSepaLink($realOrderID){
//			$base_path = Mage::getBaseDir('base');
//			$path = $base_path."/media/Sepa_Gtuschrift/Gutschrift_".$realOrderID.".xml";
//			if(file_exists($path)){
//				$rootLink = str_replace("/index.php","",Mage::getUrl('media'));
//				$xmlURL = $rootLink ."Sepa_Gtuschrift/Gutschrift_".$realOrderID.".txt";
//				return "<input type='hidden' id='sepa_link_credit' value='".$xmlURL."'>";
//			}
//		}
		...
		public function ini_Sepa_XML($name_kunde, $kundennr, $iban_kunde, $bic_kunde, $name_shop, $iban_shop, $bic_shop, $orderNr, $refundSumme){
			...
			$defaultTermin = $ini_data->modify('+1 day')->format("Y-m-d");
...
//			$base_path = Mage::getBaseDir('base');
//			file_put_contents($base_path."/media/Sepa_Gtuschrift/Gutschrift_TEST.txt", $xml);
		return $xml;
